...
- Check for typos to verify probable errors. Then run to see if it works.Compile with PEP8 for easier reading. So it s easier to others to commit.
- Containers are virtual enviroments that are attributed with the desired tools depending on the project (meaning their own imports, modulesm packages) without including the actual local system. 
